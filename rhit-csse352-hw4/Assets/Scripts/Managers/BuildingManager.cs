...
public class BuildingManager : PurchasableManager<BuildingDisplay, BuildingInfo>
{
    private static readonly ICollection<BuildingInfo> infos = new List<BuildingInfo>()
    {
        new BuildingInfo("Collectible Rocks",
                ...
                new PurchaseInfo(1, 0, 1.05f, 0f), new List<IGameModifier>{ new IncomeModifier(0.1f) }),
        new BuildingInfo("Lava Tours",            "<i>Only 3 casualties per week!</i>",
                new PurchaseInfo(20, 0, 1.05f, 0f), new List<IGameModifier>{
                    new IncomeModifier(0.5f),
                    new ObsidianRewardModifier(0.2f)
                }),
        new BuildingInfo("Basaltic Sculptures", "The number of poses is innumerable.",
                new PurchaseInfo(80, 0, 1.1f, 0f), new List<IGameModifier>{
                    new IncomeModifier(2f),
                    new ObsidianRewardModifier(1f)
                }),
        new BuildingInfo("Geothermal Generators", "Gotta keep the lights on somehow.",
                new PurchaseInfo(100, 1, 1.1f, 0f), new List<IGameModifier>{
                    new IncomeModifier(20f),
                    new ObsidianRewardModifier(1f)
                }),
        new BuildingInfo("Mineral Extractors", "<i>Shiny!</i>",
                new PurchaseInfo(500, 20, 1.5f, 10f), new List<IGameModifier>{
                    new ObsidianModifier(1f),
                    new ObsidianRewardModifier(10f)
                }),
    };
